...
def add_early_event_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["flag_usage_drop_40pct"] = (
        df["usage_count_last_1m"] < 0.4 * df["usage_count_last_3m"]
    ).astype(int)

    df["flag_feature_collapse"] = (df["active_features_last_1m"] <= 1).astype(int)

    df["flag_error_spike"] = (
        df["error_count_last_1m"] > 2 * df["error_count_last_3m"]
    ).astype(int)

    df["flag_recent_inactivity"] = (
        (df["months_since_last_usage"] >= 1) &
        (df["months_since_first_usage"] <= 3)
    ).astype(int)
...
def main():
    ...
    modeling_table = add_account_relative_features(modeling_table)
    modeling_table = add_early_event_flags(modeling_table)
